...
void buildCoOccurrenceMatrix() {
	...
	for (const auto& user : users)
	{
        ...
        for (size_t i = 0; i < interestScores.size(); i++) {
            for (size_t j = i; j < interestScores.size(); j++) {  // j 从 i 开始，包含对角线
...
void buildSimilarityMatrix() {
    ...
    // 计算相似度矩阵
    // 对角线值已经在 coOccurrenceMatrix 中计算好了
    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            ...
            // 使用共现矩阵的对角线值作为归一化因子
            double D_i = coOccurrenceMatrix[i][i];  // 商品i的"自共现"强度
            double D_j = coOccurrenceMatrix[j][j];  // 商品j的"自共现"强度
